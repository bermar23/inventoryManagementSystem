 ...
 */
class Trans_delivery_model extends CI_Model{
    function __construct(){
        parent::__construct();
    }

    public function count(){
        $result = $this->db->query("SELECT transactionSummaryID FROM tbltransactionsummary WHERE tbltransactionsummary.transactionTypeID = '5' and tbltransactionsummary.status='U';");
    ...
    public function get_unapprove(){
        $result = $this->db->query("SELECT
                                    ...
                                    WHERE
                                        summary.transactionTypeID = '5' and summary.status = 'U'
                                    GROUP BY
    ...
    public function get_details(){
        ...
        //Get Item details        
        $result_items = $this->db->query("SELECT tbltransaction.transactionID, tbltransaction.itemCode, tblitem.itemName, tbltransaction.fromQtyConsigned AS qtyConsigned, tbltransaction.fromLocationID, tbltransaction.toLocationID
                                                FROM
                                                    tbltransaction
                                                INNER JOIN
                                                    tblitem
